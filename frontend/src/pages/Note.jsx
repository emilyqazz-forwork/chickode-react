...
export function Note({ t }) {
  ...
  const getFilteredAndSorted = () => {
    let filtered = activeChapter === "all"
        ? wrongItems
        : wrongItems.filter(a => String(a.chapter) === String(activeChapter));
        ...
        <div className="book-content">
          <aside className="chapter-sidebar">
            ...
            <div id="chapterSidebar">
              <button className={`chapter-btn ${activeChapter === 'all' ? 'active' : ''}`} onClick={() => setActiveChapter('all')}>
                전체 ({wrongItems.length})
              </button>
          ...
          <section className="wrong-answer-area">
            ...
            {displayedList.length === 0 && (
              <div className="tip-box">
                아직 오답이 없어! 메인에서 문제를 풀고 틀리면 여기에 자동으로 저장돼.
              </div>
            )}
                ...
                <div key={a.id} className="wrong-card">
                  ...
                  {a.type === 'multiple' || a.type === 'ox' ? (
                    <div className="answer-compare">
                      <div className="answer-box my-answer">
                    ...
                    <div className="code-compare">
                      <div className="code-box">
                        ...
                        <pre><code>{a.userCode || "(없음)"}</code></pre>
                      </div>
                      <div className="code-box">
                        <span className="box-label">정답</span>
                        <pre><code>{a.expectedExample || "(없음)"}</code></pre>
                      </div>
                    </div>
                  )}

                  <div className="why-wrong-box">
                    <span className="box-label">왜 틀렸을까?</span>
                    {getWhyWrong(a)}
                  </div>
                  <div className="tip-box">{getTip(a)}</div>
                  <div className="action-btn-group">
                    {/* navigate back to play with single problem option */}
                    <button className="action-btn retry-btn" onClick={() => navigate('/play', { state: { singleProblemId: a.problemId || a.title } })}>다시 풀기</button>
                    <button className="action-btn ai-btn" onClick={() => alert('AI에게 질문 기능은 아직 리액트 마이그레이션 중이야 삐약!')}>AI에게 질문</button>
                  </div>
                </div>
              ))}
            </div>
